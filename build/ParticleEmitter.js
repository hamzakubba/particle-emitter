...
const getMap = (array = []) => new Map(array.map(elem => typeof elem === 'string' ? [elem, elem] : elem === null ? ['', null] : elem).map(([key, val]) => Array.isArray(key) ? key.map(innerKey => [innerKey, val]) : [[key, val]]).reduce((accumulator, currentValue) => {
  ...
  getRuleCss({
    ...
    */
    if (repeatForMaps.size) {
      ...
      const looper = (loops = [], depth = 0, maxDepth = loops.length, loopStack = []) => {
        if (depth < maxDepth) {
          ...
          const replacements = [['prefix', prefix], ['suffix', suffix], ['mediaQuery', mediaQuery]];
    ...
    mapWithDefaultTemplatesMap.forEach((value, key) => {
      ruleCss += `${indent}${this.defaultClassTemplate} { ${this.defaultRuleTemplate} }\n`.replace('$prefix', prefix).replace('$suffix', suffix).replace('$mapKey', key).replace('$mapValue', value).replace('$mediaQuery', mediaQuery);
    });
